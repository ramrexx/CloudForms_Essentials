=begin
  list_openstack_interface_port_ids.rb

  Author: Dave Costakos <david.costakos@redhat.com>

  Description: List the neutron ports associated with a VM
-------------------------------------------------------------------------------
...
begin
  ...
  response = call_rest("#{nova_url["href"]}/os-interface", :get, headers, nil)
  require 'json'
  log(:info, "Response: #{JSON.pretty_generate(JSON.parse(response))}")
  port_hash = {}
  port_list = JSON.parse(response)
  port_list["interfaceAttachments"].each { |port|
    ip_addr = port["fixed_ips"].first["ip_address"]
    port_hash[port["port_id"]] = ip_addr
  }
  $evm.object["sort_by"] = "description"
  $evm.object["sort_order"] = "ascending"
  $evm.object["data_type"] = "string"
  $evm.object["required"] = "true"
  $evm.object['values'] = port_hash
