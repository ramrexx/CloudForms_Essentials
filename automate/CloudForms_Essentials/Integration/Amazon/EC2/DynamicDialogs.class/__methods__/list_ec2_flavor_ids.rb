=begin
 list_ec2_flavor_ids.rb

 Author: Kevin Morey <kevin@redhat.com>

 Description: This method lists Amazon flavor ids
-------------------------------------------------------------------------------
...
def get_provider(provider_id=nil)
  ...
  if !provider
    ...
    if provider
      ...
      bail_out('< No providers found, check RBAC/provider tags >')
    end
  end
...
def check_rbac
  ...
  log(:info, "service model rbac_enabled?: #{$evm.rbac_enabled?}")
end
...
if provider
  ...
  $evm.vmdb(:ManageIQ_Providers_Amazon_CloudManager_Flavor).all.each do |flavor|
...
if dialog_hash.blank?
  bail_out('< No flavors found, check RBAC/flavor tags >')
else
  set_values_and_exit(dialog_hash)
end
