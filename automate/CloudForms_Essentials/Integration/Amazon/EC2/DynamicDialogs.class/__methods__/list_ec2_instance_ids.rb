=begin
 list_ec2_instance_ids.rb

 Author: Kevin Morey <kevin@redhat.com>, Nate Stephany <nate@redhat.com>

 Description: This method lists Amazon instance IDs (NOT CloudForms instance ID)
-------------------------------------------------------------------------------
...
if provider
  provider.vms.each do |vm|
    next unless vm.present?
    next if vm.archived || vm.orphaned
    dialog_hash[vm.id] = "#{vm.name} on #{provider.name}"
  end
else
  # no provider so list everything
  $evm.vmdb(:ManageIQ_Providers_Amazon_CloudManager_Vm).all.each do |vm|
    next unless vm.ext_management_system || vm.present?
    next if vm.archived || vm.orphaned
    dialog_hash[vm.id] = "#{vm.name} on #{vm.ext_management_system.name}"
  end
end

if dialog_hash.blank?
  bail_out("< No vms found, check RBAC tags >")
else
  set_values_and_exit(dialog_hash)
end
