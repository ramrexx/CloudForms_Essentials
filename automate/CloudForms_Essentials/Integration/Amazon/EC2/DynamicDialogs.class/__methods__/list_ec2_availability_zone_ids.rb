=begin
 list_ec2_availability_zone_ids.rb
...
if provider
  provider.availability_zones.each do |availability_zone|
    next unless availability_zone.name
    dialog_hash[availability_zone.id] = "#{availability_zone.name} on #{provider.name}"
  end
else
